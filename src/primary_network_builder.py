...
from exceptions import PoleToPoleDistanceNotInRange, IncompleteGeometryConfigurationDict
from utils import slice_up_network_edges, get_nearest_points_in_the_network
from networkx.algorithms import approximation as ax
import networkx as nx
from constants import (MIN_ADJUSTMENT_FACTOR, MAX_ADJUSTMENT_FACTOR, MIN_POWER_FACTOR, MAX_POWER_FACTOR,
    MIN_PERCENTAGE, MAX_PERCENTAGE, MIN_YEAR_OPERATION, MAX_YEAR_OPERATION, OVERHEAD_CONDUCTOR_CATALOG_FILE,
    OVERHEAD_CONDUCTOR_CATALAOG_SCHEMA, UNDERGROUND_CONCENTRIC_CABLE_CATALOG_SCHEMA, UG_CONCENTRIC_CABLE_CATALOG_FILE,
    LENGTH_CONVERTER_TO_CM)
from exceptions import (AdjustmentFactorNotInRangeError, PowerFactorNotInRangeError, NegativeKVError,
    ZeroKVError, AttributeDoesNotExistError, PercentageNotInRangeError, 
    OperationYearNotInRange, EmptyCatalog, ConductorNotFoundForKdrop)
import math
from enums import ConductorType, NumPhase, Phase
import pandas as pd
from utils import df_validator, get_distance
from line_section import (GeometryBasedLine, OverheadLineGeometry, OverheadLinewithNeutralGeometry, 
    LineGeometryConfiguration, UndergroundLineGeometry, Wire, Cable)
import uuid
import numpy as np
import copy
...
def choose_conductor(
    ...
    conductors_above_ampacity = conductors_above_ampacity.sort_values(by=['ampacity'])
    ...
    for record in conductors_above_ampacity.to_dict(orient='records'):
        ...
        deq = np.power(np.prod([abs(el[0]-el[1]) for el in itertools.combinations(x_array, 2)]) , 1/len(x_array)) if len(x_array) !=1 else record['gmrac']
        
        # Get the unit for deq
        deq_unit = geometry_configuration.unit if len(x_array) !=1 else record['gmrunit']
        
        # Convert deq to ft
        deq_ft = deq * LENGTH_CONVERTER_TO_CM[deq_unit] * 0.0328084
        
        # Convert gmr to ft
        gmr_ft = record['gmrac'] * LENGTH_CONVERTER_TO_CM[record['gmrunit']] * 0.0328084
        
        # Convert rac to ohm/mile
        r_mile = record['rac'] * LENGTH_CONVERTER_TO_CM[record['runit']] / 160934
        
        # Compute positive sequence impedance ohm/mile
        zpos = complex(r_mile, 0.12134*np.log(deq_ft/gmr_ft))

        # Current 
        current = complex(ampacity*pf, -ampacity*np.power(1-pf*pf, 1/2))

        # voltage drop
        vdrop_pct = (zpos*current).real*100*math.sqrt(3)/(kv_base*1000)
...
def geometry_based_line_section_builder(
    ...
    if conductor_type == ConductorType.OVERHEAD:
        if neutral_present:
            ...
            phase_cond_dict = choose_conductor(catalog, ampacity, k_drop, geometry_configuration, num_phase.value, kv_base)
            #catalog.loc[catalog[catalog['ampacity']>ampacity]['ampacity'].idxmin()].to_dict()
            line_geometry.phase_wire = convert_oh_cond_info_to_wire(phase_cond_dict)
            
            
            # Assuming neutral conductor would be one third of phase conductor ampacity for multi phase else same as phase conductor
            neutral_cond_dict = phase_cond_dict  if num_phase == NumPhase.SINGLE else \
                 choose_conductor(catalog, ampacity/3, k_drop, geometry_configuration, num_phase.value, kv_base)

            line_geometry.neutral_wire = convert_oh_cond_info_to_wire(neutral_cond_dict)
        ...
        cable_dict = choose_conductor(catalog, ampacity, k_drop, geometry_configuration, num_phase.value, kv_base)
...
class BaseSectionsBuilder:
    ...
    def __init__(self,
        ...
        self.overhead_conductor_catalog = pd.read_excel(OVERHEAD_CONDUCTOR_CATALOG_FILE)
        df_validator(OVERHEAD_CONDUCTOR_CATALAOG_SCHEMA, self.overhead_conductor_catalog)

        self.concentric_cable_catalog = pd.read_excel(UG_CONCENTRIC_CABLE_CATALOG_FILE)
        df_validator(UNDERGROUND_CONCENTRIC_CABLE_CATALOG_SCHEMA, self.concentric_cable_catalog)
...
class BaseNetworkBuilder(ABC):
    ...
    def __init__(self,
        ...
        if self.adjustment_factor < MIN_ADJUSTMENT_FACTOR or self.adjustment_factor > MAX_ADJUSTMENT_FACTOR:
            raise AdjustmentFactorNotInRangeError(self.adjustment_factor)

        if self.power_factor < MIN_POWER_FACTOR or self.power_factor > MAX_POWER_FACTOR:
            raise PowerFactorNotInRangeError(self.power_factor)

        if self.max_pole_to_pole_distance < MIN_POLE_TO_POLE_DISTANCE or self.max_pole_to_pole_distance > MAX_POLE_TO_POLE_DISTANCE:
            raise PoleToPoleDistanceNotInRange(self.max_pole_to_pole_distance)

        if self.planned_avg_annual_growth < MIN_PERCENTAGE or self.planned_avg_annual_growth > MAX_PERCENTAGE:
            raise PercentageNotInRangeError(self.planned_avg_annual_growth)

        if self.actual_avg_annual_growth < MIN_PERCENTAGE or self.actual_avg_annual_growth > MAX_PERCENTAGE:
            raise PercentageNotInRangeError(self.actual_avg_annual_growth)

        if self.actual_years_in_operation < MIN_YEAR_OPERATION or self.actual_years_in_operation > MAX_YEAR_OPERATION:
            raise OperationYearNotInRange(self.actual_years_in_operation)

        if self.planned_years_in_operation < MIN_YEAR_OPERATION or self.planned_years_in_operation > MAX_YEAR_OPERATION:
            raise OperationYearNotInRange(self.planned_years_in_operation)

    def compute_ampacity(self, non_coincident_peak: float, num_of_customers: int):

        """ Initial step is to compute maximum diversified demand"""
        div_factor = self.div_func(num_of_customers) if num_of_customers>1 else 1
        max_diversified_demand = non_coincident_peak/div_factor
        max_diversified_kva = max_diversified_demand*self.adjustment_factor/self.power_factor
        max_diversified_ampacity = max_diversified_kva/(1.732*self.kv_ll)

        original_max_diversified_ampacity = max_diversified_ampacity/math.pow( 1 + self.actual_avg_annual_growth/100 ,self.actual_years_in_operation)
        conductor_ampacity = original_max_diversified_ampacity*math.pow(1+ self.planned_avg_annual_growth/100, self.planned_years_in_operation)
...
class PrimaryNetworkFromRoad(BaseNetworkBuilder):
    ...
    def __init__(self,
        ...
        """ Get sliced graph"""
        self.sliced_graph = slice_up_network_edges(self.road_network.updated_network, \
            self.max_pole_to_pole_distance)

        """ Get substation node """
        self.substation_node = [n for n in get_nearest_points_in_the_network(self.sliced_graph, \
             [substation_loc])][0]
        self.substation_coords = self.sliced_graph.nodes[self.substation_node]['pos']

        trans_location_to_retain = [(t.longitude, t.latitude) for t in self.trans_cust_mapper]
        self.retain_nodes = get_nearest_points_in_the_network(self.sliced_graph, \
            trans_location_to_retain + [substation_loc])

    
    def get_primary_network(self):

        """ Alogorithm to convert road network into distribution system primary network """
        return ax.steinertree.steiner_tree(self.sliced_graph, list(self.retain_nodes.keys()))
    ...
    def get_network(self):
        ...
        if hasattr(self, 'network'):
            ...
            relable_snode = '_'.join(self.substation_node.split('_')[:2]) + '_ltnode'
            mapping_dict[self.substation_node] =  relable_snode
            relabel_rnodes[relable_snode] = self.retain_nodes[self.substation_node]
            self.network = nx.relabel_nodes(self.network, mapping_dict)
            self.retain_nodes = relabel_rnodes
            
            return self.network
        else:
            raise AttributeDoesNotExistError("Please call 'update_network_with_ampacity' \
    ...
    def update_network_with_ampacity(self):
        ...
        for node, node_dict in self.retain_nodes.items():
            for trans, cust_list in self.trans_cust_mapper.items():
                if trans.longitude == node_dict['centre'][0] and trans.latitude == node_dict['centre'][1]:
        ...
        for edge in dfs_tree.edges():
            ...
            distance = nx.resistance_distance(self.network, self.substation_node, edge[1])
            ...
            for node in subgraph.nodes():
                if node in transformer_nodes:
                    ...
                    noncoincident_kws += sum([l.kw for l in transformer_nodes[node]])
