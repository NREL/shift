...
from exceptions import (LatitudeNotInRangeError, LongitudeNotInRangeError, NegativeAreaError, 
    FileNotFoundError, NotCompatibleFileError, ValidationError)
...
class BuildingsFromPoint(OpenStreetBuildingGeometries):
    ...
    def get_gdf(self):
        return ox.geometries_from_point(self.point,{"building": True}, dist=self.max_dist)
...
class BuildingsFromPlace(OpenStreetBuildingGeometries):
    ...
    def get_gdf(self):
        return ox.geometries_from_address(self.place, {"building": True}, dist=self.max_dist)
