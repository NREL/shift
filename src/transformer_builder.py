...
from constants import (MIN_POWER_FACTOR, MAX_POWER_FACTOR, MIN_ADJUSTMENT_FACTOR, 
    MAX_ADJUSTMENT_FACTOR, TRANSFORMER_CATALAOG_SCHEMA, MIN_PERCENTAGE, MAX_PERCENTAGE,
    MIN_YEAR_OPERATION, MAX_YEAR_OPERATION, TRANSFORMER_CATALOG_FILE)
from enums import NumPhase, TransformerConnection, Phase
from exceptions import (PowerFactorNotInRangeError, AdjustmentFactorNotInRangeError, 
    ZeroKVError, NegativeKVError, HTkVlowerthanLTkVError, EmptyCatalog,
    MultipleCatalogFoundError, PercentageNotInRangeError, OperationYearNotInRange)
...
class TransformerLoadMapper(ABC):
    ...
    def __init__(self,
        ...
        if catalog_type != 'all':
            self.trans_catalog = self.trans_catalog.loc[self.trans_catalog['type']==type]
            if self.trans_catalog.empty:
                raise EmptyCatalog(f"Catalog of type {type} not found in transformer.xlsx file")

    
    def compute_transformer_kva_capacity(self, non_coincident_peak: float, num_of_customers: int):

        """ Initial step is to compute maximum diversified demand"""
        div_factor = self.diversity_factor_func(num_of_customers) if num_of_customers>1 else 1
        ...
        original_max_diversified_kva = max_diversified_kva/math.pow( 1 + self.actual_avg_annual_growth/100 ,self.actual_years_in_operation)
        trafo_size = original_max_diversified_kva*math.pow(1+ self.planned_avg_annual_growth/100, self.planned_years_in_operation)
        return trafo_size
    ...
    def get_catalog_object(self, t_loads: List[Load], ht_kv, lt_kv):
        ...
        kva = self.compute_transformer_kva_capacity(sum_of_noncoincident_peaks, len(t_loads))
        ...
        if not catalog.empty:
            ...
            if not catalog.empty:
                ...
                if not catalog.empty:
                    catalog_used = catalog.loc[catalog['kva'].idxmin()].to_dict()
                else:
                    raise EmptyCatalog(f"Catalog does not exist for transformer of capacity above or equal to \
                        {kva} kVA with high tension voltage above or equal to {ht_kv} and \
                        above or equal to low tension voltage{lt_kv}")

            else:
                raise EmptyCatalog(f"Catalog does not exist for transformer of capacity greater than \
                    {kva} kVA with high tension voltage greater than or equal to {ht_kv}")
        
        else:
            raise EmptyCatalog("Catalog does not exist for transformer of capacity above or equal to {kva} kVA")
...
class SingleTransformerBuilder(TransformerLoadMapper):
    ...
    def get_transformer_load_mapping(self) -> dict:
        ...
        self.transformers = {}
        catalog_used = self.get_catalog_object(self.loads, self.ht_kv, self.lt_kv)
        trans = Transformer()
...
class ClusteringBasedTransformerLoadMapper(TransformerLoadMapper):
    ...
    def get_transformer_load_mapping(self) -> dict:
        ...
        for xfmr, t_loads in cluster_to_customers.items():
            ...
            catalog_used = self.get_catalog_object(t_loads, self.ht_kv, self.lt_kv)
            trans = Transformer()
