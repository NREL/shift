...
OVERHEAD_CONDUCTOR_CATALOG_FILE = os.path.join(os.path.dirname(__file__), \
            'catalogs/overhead_conductors.xlsx')

UG_CONCENTRIC_CABLE_CATALOG_FILE = os.path.join(os.path.dirname(__file__), \
            'catalogs/concentric_neutral_ug_cables.xlsx')

TRANSFORMER_CATALOG_FILE = os.path.join(os.path.dirname(__file__), \
            'catalogs/transformer.xlsx')


MAP_STYLES = ["white-bg", "open-street-map", "carto-positron", 
            "carto-darkmatter", "stamen-terrain", "stamen-toner", 
            "stamen-watercolor","basic", "streets", "outdoors", 
...
PLOTLY_FORMAT_SIMPLE_NETWORK = {
    'nodes': {
        'node' : {'color': 'blue', 'size':7}
    },
    'edges': {
        'edge': {'color': 'red', 'size': 10}
    }
}