...
def df_validator(scehma, df):
    ...
    for idx, record in enumerate(df.to_dict(orient='records')):
        if not csv_validator.validate(record):
            errors.append(f"Item {idx}: {csv_validator.errors}, Record: {record}")
    if errors:
...
def get_point_from_curve(curve, x):
    ...
    index = sum(x_<=x)
    if index == len(x_):
        y =(y_[index-1]-y_[index-2])*(x-x_[index-2])/(x_[index-1]-x_[index-2])+y_[index-2]
    elif index == 0:
        y = (y_[index+1]-y_[index])*(x-x_[index])/(x_[index+1]-x_[index])+y_[index]
    else:
        y = (y_[index]-y_[index-1])*(x-x_[index-1])/(x_[index]-x_[index-1])+y_[index-1]
...
def get_distance(point1, point2, latlon=False):
    ...
    if not latlon:
        return  geopy.distance.distance((point1[1], point1[0]), (point2[1], point2[0])).km*1000
    else:
...
def get_nearest_points_in_the_network(graph, points):
    ...
    nearest_points = {}
    graph_node_data = {key: val['pos'] for key, val in dict(graph.nodes(data=True)).items()}
    for point in points:
...
def slice_up_network_edges(graph: nx.Graph, slice_in_meter: float) -> nx.Graph:
    ...
    graph_nodes = {x[0]: x[1]['pos'] if 'pos' in x[1] else [x[1]['x'], x[1]['y']]  for x in graph.nodes.data()}
    
    for edge in graph.edges():
        
        edge_length= get_distance((graph_nodes[edge[0]][0],graph_nodes[edge[0]][1]), (graph_nodes[edge[1]][0], graph_nodes[edge[1]][1]))
        edge_slices = [x/edge_length for x in np.arange(0, edge_length, slice_in_meter)] + [1]
        ...
        for slice in edge_slices:
            ...
            sliced_graph.add_node(f"{new_x}_{new_y}_node", pos=(new_x, new_y), type='node', data={})
            sliced_nodes.append(f"{new_x}_{new_y}_node")

        for i in range(len(sliced_nodes)-1):
            sliced_graph.add_edge(sliced_nodes[i], sliced_nodes[i+1], type='edge')
...
def get_forbidden_polygons(shp_file):
    ...
    for feature in shape.shapeRecords():
        ...
        if feature_object['type'] == 'Polygon':
            forbidden_polygons.append(shapely.geometry.Polygon(feature_object['coordinates'][0]))

    return forbidden_polygons

def get_slices(start, end, num_steps):

    return [start + i*(end-start)/(num_steps) for i in range(num_steps+1)]
    

def create_rectangular_mesh_network(lower_left: tuple, upper_right: tuple, vertical_space_meter: float = 32, horizontal_space_meter: float = 32, forbidden_areas=None, node_append_str=None):
    ...
    print(f"Vertical distance {vertical_distance}m, Horizontal distance {horizontal_distance}m")
    

    # Compute number of sections required in horizontal (wet-east) and vertical (north-south) direction
    horizontal_sections = max(int(horizontal_distance/horizontal_space_meter),1)
    vertical_sections = max(int(vertical_distance/vertical_space_meter),1)
    print(f"Vertical sections: {vertical_sections}, horizontal sections: {horizontal_sections}")
    ...
    # Let's plot the mesh
    points = {key: val['pos'] for key, val in dict(graph.nodes(data=True)).items()}

    # Let's see if the road_network exists
    
    try:
        road_ = RoadNetworkFromPolygon([north_west, north_east, south_east, south_west, north_west])
        ...
        sliced_road = nx.relabel_nodes(sliced_road, {n : n + node_append_str for n in sliced_road.nodes()})

        # Let's loop through sliced road nodes and remove closer nodes
        sliced_road_nodes = {key: val['pos'] for key, val in dict(sliced_road.nodes(data=True)).items()}
    ...
    if forbidden_areas != None:
        ...
        customer_polygon = shapely.geometry.Polygon([north_west, north_east, south_east, south_west, north_west])
...
def mesh_pruning(mesh_graph, customers):
    ...
    points = {key: val['pos'] for key, val in dict(mesh_graph.nodes(data=True)).items()}
    nodes_to_keep = []
    ...
    for customer in customers:
        ...
        customer_to_node_mapper[f"{customer[0]}_{customer[1]}_customer"] = nearest_node
...
def triangulate_using_mesh(customers, forbidden_areas=None, node_append_str=None):
    ...
    graph, points = create_rectangular_mesh_network((min(lons), min(lats)), (max(lons), max(lats)), forbidden_areas=forbidden_areas, node_append_str=node_append_str)
