...
from load_builder import (RandomPhaseAllocator, SimpleVoltageSetter, DefaultConnSetter, ConstantPowerFactorBuildingGeometryLoadBuilder, LoadBuilderEngineer)
...
loads = []
for g in geometries:
    builder = ConstantPowerFactorBuildingGeometryLoadBuilder(g, rpa, pbacc, vs, cs, 1.0)
...
for trans, cust_list in t.items():
    ...
    sc = SecondarySectionsBuilder(
        ...
        ConductorType.OVERHEAD,
        {
            NumPhase.THREE: HorizontalThreePhaseNeutralConfiguration(9, 0.4, 9.4, 'm')
        },
        {
...
lw = ConstantPowerFactorLoadWriter(loads, load_to_node_mapping_dict, 'loads.dss')
