...
from shift.network.sources.openstreet import get_road_network_from_polygon
...
def create_rectangular_mesh_network(
    ...
    try:
        road_ = get_road_network_from_polygon(
...
def mesh_pruning(
    ...
    for node in graph_mst.nodes:
        graph_mst.nodes[node]['loc'] = Location(
            latitude=points[node][1],
            longitude=points[node][0]
        )
